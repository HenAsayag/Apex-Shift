...
The game uses **Three.js WebGL 2** on desktop and mobile. Touch driving accelerates automatically: steer with the two left-side arrows (slide between them), and use Boost or Brake on the right. Brake while steering to drift. Reset Car is in the pause menu. The mobile home screen offers a circuit selector and a large Play button. See [mobile control design](docs/mobile-controls.md). Rendering uses a lower pixel-ratio cap and disables multisample antialiasing on touch devices to reduce GPU load. Landscape is recommended; the HUD also adapts to portrait and safe-area insets.

Before racing, choose **Full Screen** or **Using iPhone? Play without Full Screen**. The iPhone option allows browser play without the Fullscreen API and stays selected for the current page session, including pause/resume, restarts, and new races. Leaving fullscreen pauses the race unless browser play was explicitly selected. Landscape is recommended on phones. Automated mobile tests emulate Chromium touch devices; physical iPhone/Safari performance still needs device testing.
